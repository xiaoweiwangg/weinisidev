<template>
  <div>
    ...
    <van-search v-model="value" placeholder="输入用户名查看单个用户报表" show-action shape="round" @search="onSearch">
...
export default {
  ...
  mounted(){
      this.axios.post("/findxiaji",{name:JSON.parse(sessionStorage.getItem("userinfo")).name})
      .then((x)=>{
          console.log(x.data);
